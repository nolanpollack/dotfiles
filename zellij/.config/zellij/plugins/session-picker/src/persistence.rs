...
impl SnapshotStore {
    pub fn at(path: impl Into<PathBuf>) -> Self {
        ...
        let temp_path = path.with_file_name(format!(".{file_name}.{}.tmp", writer_nonce()));
        Self {
            path: Some(path),
...
// A per-store temp path prevents concurrent writers from sharing the atomic-write staging file.
fn writer_nonce() -> u128 {
    SystemTime::now()
...
mod tests {
    ...
    fn round_trip_and_unchanged_write_suppression() {
        ...
        let mut store = SnapshotStore::at(&path);
        let state = PersistentState::new(ThemePalette::default(), AppSnapshot::default());
        assert!(store.save_if_changed(&state).unwrap());
        assert!(!store.save_if_changed(&state).unwrap());

        let mut restored = SnapshotStore::at(&path);
        assert_eq!(restored.load().unwrap(), Some(state));
        let _ = fs::remove_file(path);
    }

    #[test]
    fn corrupt_and_unsupported_snapshots_are_nonfatal() {
        let corrupt = test_path("corrupt");
        fs::write(&corrupt, b"not-json").unwrap();
        assert_eq!(
            SnapshotStore::at(&corrupt).load().unwrap_err().kind(),
            io::ErrorKind::InvalidData
        );
        let _ = fs::remove_file(corrupt);

        let old = test_path("old");
        fs::write(&old, br#"{"schema_version":1}"#).unwrap();
        assert_eq!(SnapshotStore::at(&old).load().unwrap(), None);
        let _ = fs::remove_file(old);

        let future = test_path("future");
        fs::write(&future, br#"{"schema_version":999}"#).unwrap();
        assert_eq!(SnapshotStore::at(&future).load().unwrap(), None);
